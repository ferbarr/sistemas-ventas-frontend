    ...
    <v-dialog persistent v-model="dialog" max-width="700px">
      <v-card>
        ...
        <v-card-actions>
          ...
          <v-spacer></v-spacer>
          <v-btn @click="guardarProveedor()" v-if="this.btn.agregar"
            >Guardar</v-btn
          >
          <v-btn @click="editarProveedor()" v-if="this.btn.editar">Editar</v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
    <!-- MODAL ESTADO -->
        ...
        <v-card-text>
          <v-container>
            <span v-if="this.estado.estado === 1" class="black--text"
              >¿Deseas desactivar el proveedor
              {{ this.estado.nombre }}?</span
            >
            <span v-if="this.estado.estado === 0" class="black--text"
              >¿Deseas activar el proveedor {{ this.estado.nombre }}?</span
            >
          </v-container>
        </v-card-text>
        <v-card-actions>
...
export default {
  ...
  methods: {
    ...
    async guardarProveedor() {
      ...
      let header = { headers: token };
      try {
        const res = await this.axios.post("persona/add-persona",this.inputs,header);
        this.snack = {
          color: "success",
          time: 3500,
          active: true,
    ...
    async editarProveedor() {
      ...
      try {
        const res = await this.axios.put(
          ...
          {
            _id: this._id,
            tipo_persona:this.inputs.tipo_persona,
    ...
    async cambiarEstado() {
      ...
      if (this.estado.estado === 1) {
        try {
          const res = await this.axios.put("persona/deactivate-persona",
            {
              _id: this.estado._id,
            },
            header
